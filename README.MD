...
- `preferJsonSchemaResponses` attribute forces to use only the default json schema declarated in `success` responses.
...
6. Inputs may specify _where_ the input should be included within the generated Swagger using the key
...
1. Adding an object with the key `swagger` to individual models e.g. `api/models/modelName.js`:
   ...
   - In additon to the model's schema, the key `swagger.modelSchema` may be used to specify
     ...
     This is a _non-standard_ convenience function i.e. in Swagger/OpenAPI you need to
...
action documentation to exclude the model _completely_ (exclude the **schema**) or a specific
...
OpenAPI 3 specifies the **_Any Type_** by the absence of the `type` property in a schema;
...
All `swagger` objects may contain the elements `tags` and `components`(_except the ones specified in `config.routes.js_) e.g.
...
Note that a final _clean-up_ phase is run after processing, which performs the following:
...
1. Parameters values are Swagger definitions, with the exception of the _special_ string value
